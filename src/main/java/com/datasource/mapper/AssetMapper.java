...
public interface AssetMapper extends TSBaseMapper<Asset> {
    ...
    @Select("SELECT a.AssetName,a.Amount,a.Currency,a.Date,a.MarketValue,a.Cost,a.Profit,u.UserName,ac.CategoryName FROM Asset a LEFT JOIN User u ON u.UserID = a.AssetID LEFT JOIN AssetCategory ac ON a.CategoryID = ac.CategoryID where u.UserID = #{UserID} and a.CategoryID = #{CategoryID} and a.Date >= #{StartDate} and a.Date <= #{EndDate}")
